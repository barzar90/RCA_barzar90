<%@ Page Title="" Language="C#" MasterPageFile="~/Masters/Forms/AppForms.Master" AutoEventWireup="true" CodeBehind="MenuList.aspx.cs" Inherits="RCA.Admin.MenuManagement.MenuList" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="FormsPH" runat="server">
    <%if (Session["UserRole"] != null && Session["UserRole"].ToString().ToLower() != "cmscityuser")
        { %>
     <div class="admintab">
        <asp:Button ID="btnAddMenu" runat="server" CssClass="btn btn-primary btn-sm" Text="Add Menu"
            OnClick="btnAddMenu_Click"/>
        <asp:Button ID="btnPublish" runat="server" CssClass="btn btn-primary btn-sm" Text="Publish Menu"
            OnClick="btnPublish_Click" />
        <asp:Button ID="btnAddPlaceholder" runat="server" CssClass="btn btn-primary btn-sm"
            Text="Add Place Holder Content" OnClick="btnAddPlaceholder_Click"  />
        <asp:Button ID="btnPlaceHolderList" runat="server" CssClass="btn btn-primary btn-sm"
            Text="Place Holder Content List"  OnClick="btnPlaceHolderList_Click" />
                <asp:Button ID="btnGenSiteMap" runat="server" CssClass="btn btn-primary btn-sm" Text="Generate Site Map"
            OnClick="btnGenSiteMap_Click" />
         <asp:Button ID="btnViewCases" runat="server" CssClass="btn btn-primary btn-sm"
            Text="View Case Details"  OnClick="btnViewCases_Click" />
    ...
    <asp:GridView CssClass="table table-bordered table-striped" ID="grdManuList" AllowPaging="True"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="Menu Link">
                <ItemTemplate>
                    <%# "["+ Request.Url.OriginalString.Replace(Request.class="site-sec"Url.PathAndQuery, string.Empty)+"/"+Convert.ToString(Eval("MenuID")) + "/" + Convert.ToString(Eval("MenuName")).TrimEnd().TrimStart().Replace(" ","-") +"]" %>
                </ItemTemplate>
